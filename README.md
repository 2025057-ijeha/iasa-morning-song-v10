...
## v10.4.2 변경사항
- 안전검사에서 '가장 비슷한 곡'을 임의로 선택하던 문제 수정
- 곡 제목과 가수 모두 충분히 일치한 음원만 Explicit 판정에 사용
- 한국 가수명이 음원 서비스에서 영문 표기로 등록된 경우 YouTube 영상 제목을 보조 근거로 사용
- 정확한 곡을 확인하지 못하면 다른 노래로 검사하지 않고 자동검사 불가로 처리
- 학번 입력 예시를 `20210`으로 변경


## v10.5 변경사항
- 자동검사 결과를 `자동검사 통과 / 관리자 검사 필요 / 관리자 확인 완료 / 차단 판정`으로 구분
- Explicit 또는 금지표현이 확인된 곡은 신청 불가
- 정확한 곡을 못 찾음, 외부 검사 장애, YouTube 확인 실패, 검색 결과 충돌 등은 신청 가능하되 `관리자 검사 필요`로 저장
- 관리자 신청곡 카드에 검사 상태 배지 표시
- 관리자가 `다시 자동검사` 또는 `관리자 확인 완료` 처리 가능
- Apple/iTunes + Deezer 두 음원 카탈로그를 함께 사용해 검색/Explicit 판정 정확도 개선
- 한국 곡을 위해 KR 카탈로그 우선 검색 + 한국어/영문 아티스트 표기를 YouTube 제목/채널로 보조 확인
- 외부 API 호출은 15분 캐시하여 같은 곡 반복 검사 시 과도한 요청을 줄임
- 엑셀 내보내기에 `검사 상태` 열 추가

### 중요: 기존 Supabase 사용자는 1회 SQL 실행 필요
`supabase-v10.5-migration.sql`의 전체 내용을 Supabase > SQL Editor에 붙여넣고 Run 하세요.
